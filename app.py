...
<X3D profile="Interchange" version="3.3" xmlns:xsd="http://www.w3.org/2001/XMLSchema-instance" xsd:noNamespaceSchemaLocation="http://www.web3d.org/specifications/x3d-3.3.dtd">
  <Scene>
    <Background skyColor="0.1 0.1 0.15"/>
    <DirectionalLight direction="-1 -1 -1" intensity="0.8"/>
    <DirectionalLight direction="1 0.5 1" intensity="0.4"/>
  </Scene>
</X3D>
"""
...
def parse_prompt(text: str) -> dict:
    ...
    if "sphere" in t or "ball" in t or "orb" in t:
        ...
        shape = "cone"
    elif "cylinder" in t or "tube" in t:
        shape = "cylinder"
...
def build_shape_xml(shape, color, position, size):
    ...
    return f"""    <Transform translation="{x:.3f} {y:.3f} {z:.3f}">
      <Shape>
        {geo}
        <Appearance>
          <Material diffuseColor="{r:.3f} {g:.3f} {b:.3f}" specularColor="0.3 0.3 0.3" shininess="0.5"/>
        </Appearance>
      </Shape>
    </Transform>
"""
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .viewport{flex:1;background:#111;position:relative}
    ...
    .log-label{font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;margin-bottom:4px;opacity:.7}
...
<body>
  ...
  <div class="main">
    <div class="viewport">
      <x3d-canvas id="x3dCanvas"></x3d-canvas>
    </div>
    <div class="sidebar">
  ...
  <script>
    ...
    // Initial load
    window.addEventListener('load', () => reloadScene());
